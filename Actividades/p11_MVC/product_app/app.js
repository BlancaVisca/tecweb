...
        }
        ...
        console.log("Datos enviados:", postData);
                const url = edit === false ? './backend/product-add.php' : './backend/producto_edit.php';
                if (edit) {
                    postData.id = id;
                }
                $.post(url, postData, (response) => {
                    ...
                    try {
                        ...
                        if (res.status === "success") {
                            ...
                            // Actualizar el contenido del contenedor con la respuesta
    ...
    $(document).on('click', '.editar', function() {
        if (confirm('¿Estás seguro de querer editar el elemento?')) {
            ...
            let id = fila.find('td').first().text();
            edit=true;
            $.post('./backend/update_producto.php', {id: id}, function(response) {
                console.log("Respuesta del servidor:", response);
